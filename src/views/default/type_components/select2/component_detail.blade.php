...
//	}
    ...
    if($datatable && $form['relationship_table']) {
        ...
        $datatable_table = explode(',',$datatable)[0];
        $datatable_field = explode(',',$datatable)[1];
        $foreignKey = isset($form['fk_1'])?$form['fk_1']: CRUDBooster::getForeignKey($table,$form['relationship_table']);
        $foreignKey2 = isset($form['fk_2'])?$form['fk_2']: CRUDBooster::getForeignKey($datatable_table,$form['relationship_table']);
        $ids = DB::table($form['relationship_table'])->where($foreignKey,$id)->pluck($foreignKey2)->toArray();
